  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gray-100">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold text-center text-gray-900 mb-12">Utvalda lägenheter</h2>
        <div className="grid md:grid-cols-3 gap-8">
          {listings.map((listing) => (
            <motion.div key={listing.id} whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}>
              <Card>
                <CardHeader>
                  <img
                ...
                <CardFooter className="flex justify-between">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => toggleFavorite(listing.id)}>
                    <Heart
                      className={`h-4 w-4 ${favorites.includes(listing.id) ? "fill-red-500 text-red-500" : "text-gray-500"}`}
                    />
                  </Button>
                </CardFooter>
              </Card>
